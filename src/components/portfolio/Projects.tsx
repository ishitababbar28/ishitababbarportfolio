    ...
    >
      <div className="grid gap-5 md:grid-cols-2">
        {projects.map((p, i) => (
          <article
            key={p.name}
            className={`${i % 2 === 0 ? "reveal-left" : "reveal-right"} tilt-card shimmer-border spotlight group relative overflow-hidden rounded-2xl border border-border bg-card/60 p-7 backdrop-blur hover:border-primary/40 hover:shadow-glow`}
